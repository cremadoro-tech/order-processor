...
def render_manual_page():
    ...
    st.subheader("毎朝の日次運用フロー")
    st.markdown("""
```
07:00  GoQから3つのCSVダウンロード（楽天 + Yahoo!/Qoo10等 + Amazon）
  ↓
07:05  このアプリに3ファイルをまとめてアップロード → 「処理を実行」
  ↓
07:06  結果確認 → 「外注先別Excelをダウンロード（ZIP）」
  ↓
07:08  ZIPを展開 → 外注先ごとのExcelを各担当者に配布
  ↓
07:15  出荷作業開始
```
""")

    with st.expander("詳しい手順（初めての方向け）", expanded=False):
        st.markdown("""
**STEP 0: GoQからCSVをダウンロード**
...
1. ブラウザでこのアプリを開く
2. 「処理」ページの「CSVアップロード」エリアに2ファイルをドラッグ＆ドロップ
3. ファイル名とサイズが表示されることを確認
4. **「処理を実行」**ボタンをクリック
5. 処理完了まで待つ（約10秒）

**STEP 2: 結果確認**

1. カテゴリ別件数を確認（ジョインティ、ゴム、おなまえスタンプ等）
2. 件数が極端に少ない/多い場合はCSVを確認
3. 「ジョインティ警告」がある場合は内容を確認

**STEP 3: Excelダウンロード**

1. **「作業指示書を生成」**ボタンをクリック
2. 生成完了後、**「作業指示書Excel」**をクリックしてダウンロード
3. 必要に応じて「全カテゴリ一括ZIP」「全件CSV」もダウンロード

**STEP 4: 外注先別に配布**

ZIP内のファイルを各外注先に配布します:

| ファイル名 | 配布先 | 内容 |
|-----------|-------|------|
| 東京製版_YYYYMMDD.xlsx | 東京製版様 | おなまえスタンプ（名入れセット） |
| 永江印祥堂_YYYYMMDD.xlsx | 永江印祥堂様 | 住所印・ゴム印・のし系 |
| アシール_YYYYMMDD.xlsx | アシール様 | ジョインティ/オスカ/のべ台/分割印/おむつ |
| ジェットストリーム_YYYYMMDD.xlsx | メーカー直 | ボールペン名入れ |
| タニエバ_YYYYMMDD.xlsx | タニエバー | キャップレスネーム印 |
| ペンペン_YYYYMMDD.xlsx | ペンペン | 名入れ鉛筆 |
| ユニパレ・ハハトコ_YYYYMMDD.xlsx | メーカー | 三菱鉛筆名入れ |
| 印鑑_YYYYMMDD.xlsx | 印鑑製造 | 印鑑/チタン/スワロ/宝石/法人 |

**補足: もう一度処理したい場合**

- ファイルを削除（×ボタン）→ 新しいCSVをアップロード → 「処理を実行」
- 過去5回分の処理履歴はページ下部に残っています
""")
...
| ボタン | 内容 | 推奨 |
|--------|------|:---:|
| **外注先別Excel（ZIP）** | 外注先ごとのExcelをZIPにまとめて出力 | **推奨** |
| 全体Excel（1ファイル） | 全カテゴリを1つのExcelに出力 | |
| カテゴリ別ZIP | カテゴリごとのCSVをZIPにまとめて出力 | |
| 全件CSV | 全データをBOM付きUTF-8のCSVで出力 | |

**外注先別Excel（ZIP）の特徴:**
- 外注先ごとにファイルが分かれる（東京製版、永江印祥堂、アシール 等）
- 各ファイル内はカテゴリ×単品/複数でシート分割
- 100行ごとにPart分割（印刷用）
...
| `vendor_mapping.json` | 外注先マッピング（10外注先×カテゴリ対応表） |
...
| 処理履歴が消えた | ブラウザを閉じた / リロードした | 現在の履歴はセッション中のみ。Excelは処理のたびにダウンロードして保存してください |
| 「読み取り専用モード」表示 | Streamlit Cloud上で実行中 | 正常。日々のCSV処理は問題なし。設定変更のみGitHub経由 |

**現在の制限事項:**
- **処理履歴**: ブラウザタブを閉じると消えます。処理結果のExcel/CSVは毎回ダウンロードしてPCに保存してください
- **設定変更**: UIで変更できますが、ページリロードで元に戻ります。恒久反映はGitHub経由で
- **将来対応予定**: Google Sheets記録 or Railway移行で履歴・設定の永続化が可能です
""")
...
""")
    ...
    not_implemented = [
        ...
        ("設定変更の永続化", "中",
         "Streamlit Cloudは読み取り専用。UIでの設定変更はセッション中のみ有効。Railway移行（月~$5）で解決可能"),
        ("PDF出力・印刷レイアウト", "保留",
         "印刷用フォーマット（フォントサイズ80-100pt、改ページ50行等）。印刷運用が確認でき次第対応"),
    ]
