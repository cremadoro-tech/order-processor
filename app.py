...
def render_manual_page():
    ...
    st.markdown("""
```
07:00  GoQからCSVダウンロード（楽天分 + 楽天Amazon以外）
  ↓
07:05  このアプリにCSVをアップロード → 「処理を実行」
  ↓
07:06  結果確認 → 「作業指示書を生成」→ Excelダウンロード
  ↓
07:10  Excelを各担当者に配布（Slack / メール / 共有フォルダ）
  ↓
07:15  出荷作業開始
```
""")
...
| 担当 | 配布するシート |
|------|-------------|
| 印鑑製造 | 印鑑/チタン/スワロ/宝石/法人 |
| 外注品（ジョインティ等） | ジョインティ/オスカ/おなまえスタンプ/のべ台 等 |
| ペン名入れ | ジェットストリーム/ペンペン/クルトガ/タニエバー |
| シール | お名前シールお名前セット |
| 印影確認 | 印影確認シート → 確認後に再処理 |
| 販売課 | 販売課シート |
